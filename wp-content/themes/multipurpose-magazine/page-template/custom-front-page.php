...
<main id="maincontent" role="main">
            ...
            <div class="text-content">
              <div class="meta-box">
                    ...
                    <?php
                      ...
                      if ($posttags) {
                        foreach($posttags as $slider_tag) {
                          $count++;
                          echo esc_attr($sep) . '<a href="'.esc_html(get_tag_link($slider_tag->term_id)).'" class="p-2">'.esc_attr($slider_tag->name).'</a>';
                          $sep = ' ';
                ...
                <?php if(get_theme_mod('multipurpose_magazine_slider_date',true) != ''){?>
                  <span class="entry-date px-2"><i class="fa fa-calendar pr-2" aria-hidden="true"></i><a href="<?php echo esc_url( get_day_link( $archive_year, $archive_month, $archive_day)); ?>"><?php echo esc_html( get_the_date() ); ?><span class="screen-reader-text"><?php echo esc_html( get_the_date() ); ?></span></a></span>
                <?php }?>
                <?php if(get_theme_mod('multipurpose_magazine_slider_time',true) != ''){?>
                  <span class="entry-time px-2"><i class="far fa-clock pr-2" aria-hidden="true"></i><?php the_time(); ?></span>
                <?php }?>
              </div>
  ...
  <?php if( get_theme_mod('multipurpose_magazine_page_title') != '' || get_theme_mod('multipurpose_magazine_category') != ''){ ?>
    <section id="top-trending" class="py-5">
      <div class="container">      
        <div class="row">
          <div class="col-lg-9 col-md-9">
            <?php if( get_theme_mod('multipurpose_magazine_page_title') != ''){ ?>
              <hr class="top-head m-0">
              <h2 class="p-2"><?php echo esc_html(get_theme_mod('multipurpose_magazine_page_title','')); ?></h2>
              <hr class="top-head m-0">
            <?php }?>
            <?php 
                  ...
                  <div class="col-lg-4 col-md-6">
                    <div class="abt-img-box">
                      ...
                      <div class="trending-cat">
                          ...
                          <?php
                            ...
                            if ($posttags) {
                              foreach($posttags as $feature_tag) {
                                $count++;
                                echo esc_attr($sep) . '<a href="'.esc_html(get_tag_link($feature_tag->term_id)).'" class="p-2">'.esc_attr($feature_tag->name).'</a>';
                                $sep = ' ';
                                if( $count > get_theme_mod('multipurpose_magazine_tags_count',2) ) break; //change the number to adjust the count
                              }
                            }
                        ...
                        <h3 class="p-3"><a href="<?php the_permalink(); ?>"><?php the_title(); ?><span class="screen-reader-text"><?php the_title(); ?></span></a></h3> 
                      </div>
                    </div>
                  </div>
                  <?php endwhile; ?>
