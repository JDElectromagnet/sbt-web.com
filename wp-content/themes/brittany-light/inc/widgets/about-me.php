		...
		public function widget( $args, $instance ) {
			...
			if ( $image ) {
				...
				echo sprintf( '<p class="widget-about-avatar"><img src="%s" alt="%s" /></p>',
					esc_url( wp_get_attachment_image_url( $image, 'brittany_light_about' ) ),
					esc_attr( $attachment['alt'] )
				);
			}

			echo wp_kses_post( wpautop( do_shortcode( $text ) ) );
					...
					<p class="entry-sig-img">
						<?php
							if ( ! empty( $signature_image ) ) {
								echo wp_get_attachment_image( $signature_image, 'post-thumbnail' );
							} elseif ( ! empty( $signature_text ) ) {
								echo esc_html( $signature_text );
							}
						?>
					</p>
				</div>
			...
			<p>
				...
				<div class="ci-upload-preview">
					<div class="upload-preview">
						<?php if ( ! empty( $image ) ) : ?>
							<?php
								$image_url = wp_get_attachment_image_url( $image, 'brittany_light_featgal_small_thumb' );
				...
				<div class="ci-upload-preview">
					<div class="upload-preview">
						<?php if ( ! empty( $signature_image ) ) : ?>
							<?php
								$image_url = wp_get_attachment_image_url( $signature_image, 'brittany_light_featgal_small_thumb' );
								echo sprintf( '<img src="%s" /><a href="#" class="close media-modal-icon" title="%s"></a>',
									esc_url( $image_url ),
									esc_attr__( 'Remove image', 'brittany-light' )
								);
