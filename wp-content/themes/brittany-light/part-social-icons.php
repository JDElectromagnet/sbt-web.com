	...
	if ( ( in_the_loop() && count( $used ) > 0 ) || ( ! in_the_loop() && ( count( $used ) > 0 || $has_rss ) ) ) {
		...
		<ul class="social-icons">
			<?php
				foreach ( $networks as $network ) {
					if ( ! empty( $used[ $network['name'] ] ) ) {
						echo sprintf( '<li><a href="%s" class="social-icon" target="%s"><i class="%s"></i></a></li>',
			...
			<?php if ( ! in_the_loop() && $has_rss ) : ?>
				<li><a href="<?php echo esc_url( get_theme_mod( 'rss_feed', get_bloginfo( 'rss2_url' ) ) ); ?>" class="social-icon" target="<?php echo esc_attr( $target ); ?>"><i class="fas fa-rss"></i></a></li>
			<?php endif; ?>
		</ul>
		<?php
	}
